...
Failed    → attempts < max AND confirmed_at IS NULL
...
**The gate is `confirmed_at IS NULL`, not the attempt count.** A payout that
...
- **A confirmed payout is never retried**, whatever its attempt count. Set
  `confirmed_at`, leave attempts at zero, and assert the worker does not
  claim it.
